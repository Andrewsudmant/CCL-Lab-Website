...
Gate 5I records owner approval of Gate 5H and the bounded Draft 0.1 release candidate in `docs/decisions/gate-5h-owner-approval-and-draft-release.md`. Do not reopen those approvals. `low-carbon-cities-affordable` is removed only from prominent Delivery examples; preserve its canonical page, featured-publication flag, complete inventory/search entry and underlying themes. Publication procedures belong on `/publications/metadata-and-sources.html`; individual pages retain full bibliographic identity, explicit peer-review status, concise lab relationship, distinctive chronology, original-source and correction links. Never alter canonical metadata to reduce public boilerplate. New audit runs must use a gate-specific output path, and tests must not overwrite historical audit reports. Gate 5I may mark PR #1 ready after passing checks (or an explicit bounded manual keyboard check); the owner alone performs merge, protection/environment setup and deployment.
