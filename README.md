...
Gate 5I records approval of that voice, removes one redundant prominent Delivery example without deleting its publication, and consolidates publication procedures on `publications/metadata-and-sources.qmd`. Canonical records stay frozen. See `docs/reviews/gate-5i/draft-0-1-final-readiness.md` and the updated publication runbook before any owner-controlled merge or deployment. Use a gate-specific `--output` when running `scripts/audit_public_voice.py`, so historical reports remain unchanged.
