...
Theme copy must be changed in `config/research_scope.yml`, not duplicated in pages. `scripts/generate_site.py` creates the homepage cycle, research overview, theme pages, project learning sections and theme links from that registry. Old `/research/themes/...` URLs are retained as generated transition pages; new internal links use `/research/<theme>.html`.
