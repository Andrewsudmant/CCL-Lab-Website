...
Final pre-push run: 26 August 2026
...
The scan reports only locations and suspected types if a match occurs; it never prints a matched value. A second scan was run after the final QA-result commit and before the first remote push. No API key was requested, accessed or stored.
