...
def generate_publications(publications: list[dict[str, Any]]) -> None:
    ...
    for year in sorted(by_year, reverse=True):
        ...
        for record in sorted(by_year[year], key=lambda item: item["title"]):
            ...
            complete.append(f'<article class="bibliography-entry" data-year="{year}" data-type="{esc(record["publication_type"])}" data-themes="{esc(themes)}"><h3><a href="{esc(site_path(f"/publications/{record['record_id']}.html"))}">{esc(record["title"])}</a></h3><p>{esc(", ".join(record["authors"]))}</p><p class="record-meta">{esc(record["venue"])} · {esc(record["publication_type"].replace("-", " ").title())} · {esc(status)} {mdpi}</p></article>')
...
Metadata sources: {public_metadata_labels(record['metadata_sources'])} · last verified {record['last_verified_date']} · status: {record['verification_status'].replace('-', ' ').capitalize()}. Dates are displayed only to the precision supplied by the verified record. No AI-generated bibliographic metadata is used.'''
