#!/usr/bin/env python3
"""Package the complete Gate 2–3A owner review artifact with checksums."""
...
def included_files() -> list[tuple[Path, Path]]:
    ...
    for relative_root, archive_root in ((Path("_site"), Path("rendered-site")), (Path("reports/screenshots"), Path("review/screenshots")), (Path("reports/qa"), Path("review/qa"))):
...
def package(output: Path | None = None) -> Path:
    destination = output or ROOT / "deliverables" / f"CCLL-owner-review-gate-2-3a-{dt.date.today().isoformat()}.zip"
    ...
    manifest = ["Cities & Climate Learning Lab — Gate 2–3A owner review", f"Created: {dt.date.today().isoformat()}", f"Branch: {branch}", f"Commit: {commit}", f"Files: {len(files)}", "", "SHA-256  Archive path"]
    with zipfile.ZipFile(destination, "w", zipfile.ZIP_DEFLATED) as archive:
        for source, target in files:
            archive.write(source, target)
            manifest.append(f"{sha256(source)}  {target.as_posix()}")
        archive.writestr("00_READ_ME_FIRST.md", (ROOT / "docs/handoffs/gate-2-3a-handoff.md").read_text(encoding="utf-8"))
